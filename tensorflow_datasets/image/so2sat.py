  ...
  def _info(self):
    if self.builder_config.selection == 'rgb':
      features = tfds.features.FeaturesDict({
          'rgb': tfds.features.Image(shape=[32, 32, 3]),
      ...
      features = tfds.features.FeaturesDict({
          'sen1': tfds.features.Tensor(shape=[32, 32, 8], dtype=tf.float32),
          'sen2': tfds.features.Tensor(shape=[32, 32, 10], dtype=tf.float32),
          'label': tfds.features.ClassLabel(num_classes=17),
          'sample_id': tfds.features.Tensor(shape=(), dtype=tf.int64),
      })
  ...
  def _generate_examples(self, path, selection):
    ...
    with h5py.File(path, 'r') as fid:
      ...
      for i in range(len(sen1)):
        if selection == 'rgb':
          yield {
              'rgb': _create_rgb(sen2[i]),
              'label': np.argmax(label[i]).astype(int),
              'sample_id': i,
          }
        elif selection == 'all':
          yield {
              'sen1': sen1[i].astype(np.float32),
              'sen2': sen2[i].astype(np.float32),
              'label': np.argmax(label[i]).astype(int),
              'sample_id': i,
          }
